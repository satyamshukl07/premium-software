...
export default function AdminSpecialistsPage() {
  ...
  const fetchSpecialists = async (page = 1) => {
    setLoading(true);
    try {
      ...
      const res = await authFetch(`/api/admin/specialist-requests?${params}`);
      if (res.ok) {
      ...
      console.error('Failed to load specialist requests:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    ...
    try {
      const res = await authFetch(`/api/admin/specialist-requests/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ status: newStatus }),
      });

      if (res.ok) {
        setSpecialists((prev) => prev.map((item) => (item.id === id ? { ...item, status: newStatus } : item)));
        if (selectedRequest && selectedRequest.id === id) {
          setSelectedRequest((prev) => ({ ...prev, status: newStatus }));
        }
      }
    } catch (err) {
      console.error('Failed to update specialist request status:', err);
    } finally {
  ...
  const handleDeleteRequest = async (id) => {
    ...
    try {
      ...
      console.error('Failed to delete specialist request:', err);
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['ID', 'Name', 'Email', 'Phone', 'Company', 'Message', 'Status', 'Date'];
    ...
    link.setAttribute('download', `autolink_specialists_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <AdminLayout title="Specialist Consultations">
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
          <form onSubmit={handleSearchSubmit} className="w-full md:w-80 relative">
            ...
            <input
              ...
              placeholder="Search specialists, client, company..."
          ...
          <div className="w-full md:w-auto flex flex-wrap items-center justify-end gap-2.5">
            ...
            <button
              type="button"
              onClick={() => fetchSpecialists(pagination.page)}
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden">
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-bold border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-6 py-3.5">Consultation Scope</th>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-400">
                      ...
                      Loading specialist consultation requests...
                    </td>
                  </tr>
                  ...
                  specialists.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4 max-w-xs">
                        <div className="truncate text-slate-800">{item.message || 'Consultation request'}</div>
                      </td>
                      <td className="px-6 py-4">
                        <select
                          value={item.status}
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-400">
                      No specialist consultation requests found.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
            <div>
              Showing <span className="font-bold">{specialists.length}</span> of{' '}
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={() => fetchSpecialists(pagination.page - 1)}
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                <div>
                  <span className="text-[10px] uppercase font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full">
                    Specialist Request #{selectedRequest.id}
                  </span>
                  <h3 className="text-xl font-bold text-slate-900 mt-1">{selectedRequest.name}</h3>
                </div>
                <button
                  type="button"
              ...
              <div className="py-6 space-y-4 text-xs">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <span className="font-bold text-slate-900">{selectedRequest.company || 'Not specified'}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block mb-0.5">Email</span>
                    <a href={`mailto:${selectedRequest.email}`} className="font-bold text-blue-600 hover:underline">
                      {selectedRequest.email}
                    </a>
                  </div>
                  <div>
                    <span className="text-slate-400 block mb-0.5">Phone</span>
                    <span className="font-bold text-slate-900">{selectedRequest.phone || 'Not provided'}</span>
                  </div>
                  <div>
                ...
                <div>
                  <label className="font-bold text-slate-700 block mb-1.5">Consultation Notes & Scope</label>
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-800 whitespace-pre-wrap">
                    {selectedRequest.message || 'No additional consultation notes provided.'}
                  </div>
                </div>
        ...
        {deleteConfirmId && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-sm w-full p-6 text-center shadow-2xl border border-slate-200">
              ...
              <h4 className="text-lg font-bold text-slate-900">Delete consultation request?</h4>
